...
struct node *start_customer = NULL,*newnode,*end_customer = NULL;
struct node *start_admin = NULL, *end_admin = NULL;
//struct node *head_s;

void adminmenu()
{
    printf("\n\t\t\t\t\t\t\t1. View List\n");
    printf("\t\t\t\t\t\t\t2. Add new movies in the List\n");
    printf("\t\t\t\t\t\t\t3. Delete movie from the List\n");
...
struct node* createadmin(struct node *start,int data , char name[25], float price)
{
...
struct node* createcustomer(struct node *start,int data,int quantity)
{
    newnode = (struct node*)malloc(sizeof(struct node));

    struct node *temp1 = start_admin;
    int flag = 0;
    while(temp1!=NULL)
    {
        if(temp1->data==data)
    ...
    if(flag==1)
    {
        ...
        if(temp==NULL)
            end_customer = start_customer = newnode;
        else
        {
            while(temp->next!=NULL)
                temp=temp->next;
            ...
            temp->next=newnode;
            newnode->prev = end_customer;
            end_customer = newnode;
        }
...
void displayList(struct node *head)
{
    ...
    else
    {
        ...
        {
            if(temp1->quantity==0)
                printf("\t\t\t\t\t\t\t%d\t%s\t%0.2f\n",temp1->data,temp1->name,temp1->price);
            else
            {
                printf("\t\t\t\t\t\t\t%d\t%s\t%d\t%0.2f\n",temp1->data,temp1->name,temp1->quantity,temp1->price);
            }
